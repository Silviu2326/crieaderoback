...
const PREVIOUS_FILE = path.resolve(process.cwd(), 'available-domains.txt');
const OUTPUT_FILE = path.resolve(process.cwd(), 'available-domains-batch2.txt');
...
async function main() {
  console.log(`🤖 Objetivo: encontrar ${TARGET_AVAILABLE} dominios .com NUEVOS para ERP fitness`);
  console.log(`💾 Guardando en: ${OUTPUT_FILE}`);

  const previousDomains: string[] = [];
  if (fs.existsSync(PREVIOUS_FILE)) {
    previousDomains.push(...fs.readFileSync(PREVIOUS_FILE, 'utf-8')
      .split('\n')
      .map((l) => l.trim())
      .filter((l) => l.length > 0 && !l.startsWith('#')));
  }
  console.log(`🚫 Excluyendo ${previousDomains.length} dominios ya encontrados en ${PREVIOUS_FILE}\n`);

  fs.writeFileSync(OUTPUT_FILE, '# Dominios .com disponibles para ERP fitness (Batch 2)\n# Generado el ' + new Date().toISOString() + '\n\n', 'utf-8');

  const checkedDomains = new Set<string>(previousDomains.map((d) => d.replace(/\.com$/, '')));
